...
void main()
{
    ...
    if (PerlinNoise3D(vec3(vs_Pos) * 2.) < .01) {
        ...
        vec4 newposition = vec4(vec4(vs_Pos.x, vs_Pos.y, vs_Pos.z, 1.f) + vs_Nor * h * .9f);
        newposition_perlin_valley = newposition;
    ...
    //BIOME 2
        ...
        //vec4 vs_Pos_fbm = vec4(vs_Pos.x - cos(u_Time), vs_Pos.y - cos(u_Time), vs_Pos.z - cos(u_Time), 1.);
        vec4 vs_Pos_fbm = vs_Pos + float(u_Time * .15f);
        //vec4 vs_Pos_fbm = vs_Pos;

        float h = fbm(vs_Pos_fbm.x * 2.f, vs_Pos_fbm.y * 2.f, vs_Pos_fbm.z * 2.f);

        // float epsilon = .000015f;
        // vec3 new_Nor = vec3(fbm(vs_Pos_fbm.x + epsilon, vs_Pos_fbm.y, vs_Pos_fbm.z) - fbm(vs_Pos_fbm.x - epsilon, vs_Pos_fbm.y, vs_Pos_fbm.z),
        //                     fbm(vs_Pos_fbm.x, vs_Pos_fbm.y + epsilon, vs_Pos_fbm.z) - fbm(vs_Pos_fbm.x, vs_Pos_fbm.y - epsilon, vs_Pos_fbm.z),
        //                     fbm(vs_Pos_fbm.x, vs_Pos_fbm.y, vs_Pos_fbm.z + epsilon) - fbm(vs_Pos_fbm.x, vs_Pos_fbm.y, vs_Pos_fbm.z - epsilon));
        // //fs_Nor = vec4(new_Nor, 0);
        //vec4 fs_Nor_mix = mix(vs_Nor, vec4(new_Nor,0.f), .99f);
        fs_Nor = vec4(invTranspose * vec3(vs_Nor), 0);

        //fs_Nor = vec4(invTranspose * vec3(vs_Nor), 0);
        //fs_Nor = vec4(invTranspose * vec3(new_Nor), 0);          // Pass the vertex normals to the fragment shader for interpolation.
                                                                // Transform the geometry's normals by the inverse transpose of the
                                                                // model matrix. This is necessary to ensure the normals remain
                                                                // perpendicular to the surface after the surface is transformed by
                                                                // the model matrix.

        vec4 newposition = vec4(vec4(vs_Pos.x, vs_Pos.y, vs_Pos.z, 1.f) + vs_Nor * h * .5f);
        newposition_fbm_cliffs = newposition;
        //newposition[1] += .5 * cos(2. * cos(.3 * u_Time) * (u_Time + newposition[0] - newposition[2] + 10.)) + .5;

        fs_Pos = newposition;

        //if (length(newposition) <= 2.7f) {
            //h = h * .75;
            //h = 0.;
            //fs_Pos = vs_Pos;
            //newposition = vs_Pos;
            //newposition = vec4(vec4(vs_Pos.x, vs_Pos.y, vs_Pos.z, 1.f) + vs_Nor * h * .75f);
        //}
    ...
    if (biome > 0.01) {
        vec4 newposition_mix = mix(newposition_perlin_valley, newposition_fbm_cliffs, .5f);
